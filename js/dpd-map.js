    ...
    initMap: function(container, settings) {
      ...
      mapPoints.forEach(point => {
        ...
        popupContent.innerHTML = `
          <h4>${this.escapeHtml(point.name)}</h4>
          <p>${this.escapeHtml(point.address)}</p>
          <p><strong>${texts.distance || 'Distance:'}</strong> ${point.distance} km</p>
          <button class="dpd-select-from-map" data-id="${point.id}">
